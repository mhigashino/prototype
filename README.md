...
機能の区切りや社内共有のタイミングで Git タグを打つことを推奨します

```bash
# タグを作成してプッシュ
git tag v1.0 -m "ウォレット画面追加"
...
GitHub 上でタグ一覧から過去バージョンのコードを参照できます

## 環境・表示について

### Pull to Refresh

ホーム・オークション・出品・おさいふタブでスクロール最上部から下に引くとリロードされます

- 引いた量に応じてコンテンツが押し下げられ、隙間にスピナーが表示される
- 閾値まで引いて離すとスピナーが回転し、最新版をリロード
- 閾値未満で離した場合はスムーズに元に戻る
