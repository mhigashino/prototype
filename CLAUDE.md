...
### CTAボタン（`.idp-bottom-bar` / `.idp-btn-buy`）のバリエーション
- 配置: タブバーの上 `bottom: calc(76px + 16px)`、`position: absolute`、フローティング
- **購入する** — Primary（赤背景 #FF333F・白文字）: 通常の商品詳細
- **入札する** — Primary: オークション画面（`_currentItemIsAuction = true`）から開いた商品詳細
- **商品を編集する** — Secondary（白背景・赤テキスト・赤ボーダー1px）: 出品画面（`_currentItemIsListing = true`）から開いた商品詳細
- **この検索条件を保存する** — Primary: 検索結果画面

### 出品画面からの商品詳細
- 出品画面（`screen-list`）から商品詳細を開いた場合、`_currentItemIsListing = true` となる
- このとき購入ボタンのラベルは「商品を編集する」に変わり、ボタンスタイルは secondary になる
...
- ×ボタン（`onSearchClose()`）: サジェスト表示中はサジェストを閉じて直前の画面に戻る（検索結果スタックがあればキーワード復元）、サジェスト非表示時は `exitSearch()` で検索画面を閉じる
